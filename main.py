...
def main(win, width):
    ...
    while run:
        ...
        global current_weather, time_input, time_weight, weather_index
        ...
        for event in pygame.event.get():
            ...
            if started:
                continue
            # Checks for any keys being pressed down
            if event.type == pygame.KEYDOWN:
                # Checks specifically for the key R being pressed down if so increasing the index_counter var
                        ...
                        for spot in row:
                            ...
                            if current_weather == snow:
                                if time_input == '00':
                                    ...
                                    time_weight = twelve_pm_snow
                    algo(lambda: draw(win, grid, ROWS, width), grid, start, end, time_weight, current_weather=clear)
...
def algo(draw, grid, start, end, time_weight, current_weather):  # Added current_weather as a parameter
    ...
    f_score[start] = h(start.get_pos(), end.get_pos()) # total score of the g and h score (which approx distance form end)
    open_set_hash = {start}
